...
test.describe('Render: AVIF + retina DPR', () => {
	test('AVIF + dpr=2 produces a 2x AVIF render, dpr=10 clamps to 3', async ({ page }) => {
		...
		// AVIF: status 200, content-type image/avif, magic bytes start with
		// 'ftypavif'/'ftypheic'/'ftypmif1' inside the ISOBMFF container at
		// bytes 4-12. We assert the content-type and a non-empty body —
		// magic-byte sniffing is overkill for an integration test.
		const avifRes = await ctx.get(avifUrl);
		...
		const dpr1Res = await ctx.get(imageUrl);
