...
Canvas logs every render request to a narrow `render_events` table — no payload, no IPs in cleartext, just daily-salted hashes — and surfaces the aggregates at [`/account/usage`](#) and [`/admin/usage`](#). One row per render: source (`on-the-fly` / `baked-api` / `baked-app` / `preview`), canvas, owner, requester, API key, format, params hash, cache-hit flag, duration, status code, optional `ip_hash`.
